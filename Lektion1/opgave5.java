...
public class opgave5 {
    ...
    public static int bedsteFerieUgerStart(int[] weeks, int antal) {
        int weekToStart = 0;
        int bestWeeksTotalRain = bedsteFerieUgerStartCalculator(weeks, antal, 0);
        for (int i = 1; i < weeks.length - antal; i++) {
            int totalRain = bedsteFerieUgerStartCalculator(weeks, antal, i);
            if(totalRain < bestWeeksTotalRain){
                bestWeeksTotalRain = totalRain;
                weekToStart = i + 1;
            }
    ...
    public stati int bedsteFerieUgerStartCalculator(int[] weeks, int antal, int firstWeek) {
        int totalRain = 0;
        for (int i = firstWeek; i < firstWeek + antal; i++) {
            totalRain += weeks[i];
        }
        return totalRain;
    }
